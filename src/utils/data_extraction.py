...
def extract_text_and_metadata(
    bucket_name: str, extract_all: bool = True
) -> List[Dict[str, object]]:
    """
    Main function to extract text and metadata from all DOCX files in a GCS bucket.
    ...
    Returns:
        List[Dict[str, object]]: A list of dictionaries containing text and metadata for all DOCX files.
    """
    ...
    try:
        ...
        # Process each selected blob
        for blob in selected_blobs:
            logger.info(f"Processing blob: {blob}")
            file_bytes = blob.download_as_bytes()  # Download the file as bytes
            ...
            extracted_data.extend(file_data)  # Append the extracted data to the result
...
# data = extract_text_and_metadata(bucket_name="team-meetings", extract_all=False)
